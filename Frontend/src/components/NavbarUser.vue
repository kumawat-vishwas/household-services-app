<template>
    <nav class="navbar border-bottom mb-3 navbar-expand-lg bg-body-tertiary" :class="{ 'nav-expanded': !navbarLinks }">
        <div class="container-fluid">
            <router-link to="/user/dashboard" class="navbar-brand">
                <p class="bold" style="margin: 0px;">Welcome to Customer Dashboard</p>
            ...
            <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
                <div class="navbar-nav">
                    ...
                    <router-link to="/user/profile" class="nav-link">
                        <p class="bi bi-person-square"><span class="mx-1">Profile</span></p>
                    </router-link>
                    <router-link to="/user/summary" class="nav-link">
                        <p class="bi bi-graph-up"><span class="mx-1">Summary</span></p>
                    </router-link>
...
export default {
    ...
    async mounted() {
        ...
        await this.checkUserBlockedStatus();
    },
    methods: {
        ...
        async checkUserPermission() {
            try {
                ...
                var alertMessage = {
                    "Alert": 'An error occurred, User permission fetching failed',
                };
        ...
        async checkUserBlockedStatus() {
            try {
                if (this.authToken) {
                    const response = await fetch('http://localhost:8080/api/checkUserBlockedStatus', {
                        method: 'GET',
                        headers: {
                            'Content-Type': 'application/json',
                            'Authentication-Token': this.authToken,
                ...
                var alertMessage = {
                    "Alert": 'An error occurred, Blocked status fetching failed',
                };
                this.$store.dispatch('updateAlertMessage', alertMessage);
                this.showBannerFor3Seconds();
            }
